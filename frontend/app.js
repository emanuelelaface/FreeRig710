...
function initStationSettings() {
  ...
  const normalizedUnsignedDecimal = (value, fallback = "0") => {
    const raw = String(value ?? "").trim();
    if (!/^\d+$/.test(raw)) return String(fallback ?? "0");
    return raw.replace(/^0+(?=\d)/, "") || "0";
  };

  const unsignedDecimalGreaterThan = (a, b) => {
    const left = normalizedUnsignedDecimal(a);
    const right = normalizedUnsignedDecimal(b);
  ...
  const runSettingsQrzSync = async () => {
    ...
    try {
      ...
      for (;;) {
        ...
        const pageCount = Number(job?.count || 0);
        const previousAfter = after;
        const nextAfter = normalizedUnsignedDecimal(job?.next_after_logid, previousAfter);
        const cursorAdvanced = unsignedDecimalGreaterThan(nextAfter, previousAfter);
        if (pageCount > 0 && pageParsed > 0 && !cursorAdvanced) {
          throw new Error(`QRZ cursor did not advance after LOGID ${previousAfter}`);
        }
...
function connectEvents() {
  ...
  let running = false;
  const poll = async () => {
    if (!running || pollPromise) return pollPromise;
    const currentGeneration = ++generation;
    const controller = new AbortController();
    pollController = controller;
    const task = (async () => {
      try {
        const state = await api("/api/v1/state", { signal: controller.signal, timeoutMs: 5000 });
        if (running && currentGeneration === generation) updateState(state);
      } catch (error) {
        if (error?.name === "AbortError" || currentGeneration !== generation || !running) return;
  ...
  const start = () => {
    ...
    pollController = null;
    pollPromise = null;
    if (!timer) timer = window.setInterval(poll, 600);
    void poll();
  };
  ...
  document.addEventListener("resume", start);
...
function initVideo() {
  ...
  const recoverAfterSystemWake = () => {
    if (document.hidden || radioPower === "OFF" || radioPower === "STARTING") return;
    const now = Date.now();
    if (now - lastWakeRecoveryAt < 500) return;
    lastWakeRecoveryAt = now;
    stop();
    retryTimer = setTimeout(load, 50);
  };
  document.addEventListener("resume", recoverAfterSystemWake);
  window.addEventListener("blur", () => { blurredSinceMs = Date.now(); });
  window.addEventListener("focus", () => {
    const blurredForMs = blurredSinceMs ? Math.max(0, Date.now() - blurredSinceMs) : 0;
    blurredSinceMs = 0;
    if (blurredForMs > VIDEO_HIDDEN_GRACE_MS) recoverAfterSystemWake();
  });
  window.addEventListener("online", recoverAfterSystemWake);
...
function initAudio() {
  ...
  const disableAudio = async (quiet = false, preserveWanted = false) => {
    if (stopping) return;
    if (!preserveWanted) audioWanted = false;
  ...
  const setupAudioOwnerChannel = () => {
    if (typeof BroadcastChannel === "undefined" || audioOwnerChannel) return;
    audioOwnerChannel = new BroadcastChannel("freerig710-audio-owner-v1");
  ...
  setupAudioOwnerChannel();

  const failAudio = async (message, quiet = false) => {
    ...
    try {
      ...
      socket.onclose = (event) => {
        ...
        if (audioReady || starting) {
          ...
          void failAudio(`Audio connection closed${suffix}`).finally(() => scheduleAudioRecovery(1200));
        }
      };
    } catch (error) {
  ...
  const recoverAudioAfterWake = async () => {
    if (audioRecoveryPromise) return audioRecoveryPromise;
    if (!audioWanted || document.hidden) return false;
    audioRecoveryPromise = (async () => {
      if (context?.state === "suspended") {
        try { await context.resume(); } catch (_) { /* reconnect below if needed */ }
      }
      await new Promise((resolve) => window.setTimeout(resolve, 300));
      const socketFresh = audioReady && socket?.readyState === WebSocket.OPEN &&
        lastAudioMessageAt && Date.now() - lastAudioMessageAt < 2000;
      if (socketFresh && context?.state === "running") return true;

      for (let attempt = 0; failureInProgress && attempt < 30; attempt += 1) {
        await new Promise((resolve) => window.setTimeout(resolve, 50));
      }
      if (failureInProgress) return false;
      await failAudio("Reconnecting radio audio after browser suspension…", true);
      for (let attempt = 0; failureInProgress && attempt < 10; attempt += 1) {
        await new Promise((resolve) => window.setTimeout(resolve, 50));
      }
      if (!audioWanted || document.hidden) return false;
      await enableAudio();
      return Boolean(audioReady);
    })().finally(() => {
      audioRecoveryPromise = null;
      if (audioWanted && !audioReady && !document.hidden) scheduleAudioRecovery(2000);
    });
    return audioRecoveryPromise;
  };

  const scheduleAudioRecovery = (delayMs = 250) => {
    if (!audioWanted || document.hidden) return;
    window.clearTimeout(audioRecoveryTimer);
    audioRecoveryTimer = window.setTimeout(() => {
      audioRecoveryTimer = null;
      void recoverAudioAfterWake();
  ...
  window.addEventListener("pagehide", () => {
    ...
    audioOwnerChannel = null;
    lastAudioMessageAt = 0;
  });
  window.addEventListener("pageshow", () => {
    setupAudioOwnerChannel();
    scheduleAudioRecovery(250);
  });
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) scheduleAudioRecovery(250);
  });
  document.addEventListener("resume", () => scheduleAudioRecovery(100));
  window.addEventListener("focus", () => scheduleAudioRecovery(250));
  window.addEventListener("online", () => scheduleAudioRecovery(100));
